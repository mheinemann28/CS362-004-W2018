...
int checkAdventureCard(int p, struct gameState *post, int deckTreasure, int failedCount);
...
int main() {
	...
	for (i = 0; i < 500; i++) {
		...
		failCheck += checkAdventureCard(p, &G, deckTreasure, failedCount);
	}
	if(failCheck == 0)
