...
int main() {
	...
	stewardEffect(1, -1, -1, &G, (G.handCount[0] - 1), 0);
	...
	expectedHandCount = G.handCount[0] - 1;
	stewardEffect(2, -1, -1, &G, (G.handCount[0] - 1), 0);
	...
	stewardEffect(3, 1, 2, &G, (G.handCount[0] - 1), 0);
	if (expectedHandCount != G.handCount[0]) {
		printf("FAILED Expected hand count: %d; Actual hand count: %d\n", expectedHandCount, G.handCount[0]);
		failedCount++;
	}
